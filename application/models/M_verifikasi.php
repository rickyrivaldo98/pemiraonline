...
class M_verifikasi extends CI_Model
{
	...
	{
		$this->db->query("UPDATE biologi SET registrasi = 0 WHERE nim = '$nim'");
	}
	
	function tolakDataBioteknologi($nim)
	{
		$this->db->query("UPDATE bioteknologi SET registrasi = 0 WHERE nim = '$nim'");
	}
	
	function tolakDataKimia($nim)
	{
		$this->db->query("UPDATE kimia SET registrasi = 0 WHERE nim = '$nim'");
	}
	
	function tolakDataFisika($nim)
	{
		$this->db->query("UPDATE fisika SET registrasi = 0 WHERE nim = '$nim'");
	}
	
	function tolakDataMatematika($nim)
	{
		$this->db->query("UPDATE matematika SET registrasi = 0 WHERE nim = '$nim'");
	}
	
	function tolakDataInformatika($nim)
	{
		$this->db->query("UPDATE informatika SET registrasi = 0 WHERE nim = '$nim'");
	}
	
	function tolakDataStatistika($nim)
	{
		$this->db->query("UPDATE statistika SET registrasi = 0 WHERE nim = '$nim'");
    }
